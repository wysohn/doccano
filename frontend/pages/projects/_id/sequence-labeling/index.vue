...
export default {
  ...
  computed: {
    ...
    shortKeys() {
      return Object.fromEntries(
        this.labels.map((item) => [item.id, [item.suffixKey]])
      );
    },
    projectId() {
  ...
  methods: {
    async list(docId) {
      ...
      const links = await this.$services.sequenceLabeling.listLinks(
        this.projectId
      );

      annotations.forEach(function(annotation) {
        annotation.links = links.filter(
          (link) => link.annotation_id_1 === annotation.id
        );
      });
    ...
    async selectTarget(chunk) {
      ...
      if (!chunk.links.find((ch) => ch.id === this.sourceChunk.id)) {
        await this.$services.sequenceLabeling.createLink(
          this.projectId,
          this.sourceChunk.id,
          chunk.id,
          this.sourceLinkType.id,
          this.getUserId
        );
        await this.list(this.doc.id);
      }
    ...
    async deleteLink(id, ndx) {
      await this.$services.sequenceLabeling.deleteLink(
        this.projectId,
        this.sourceChunk.links[ndx].id
      );
      await this.list(this.doc.id);
